...
pub struct ModuleResult {
    ...
    failure_types: LogicalPortFailures,
}
...
impl ModuleResult {
    ...
    pub fn failure_types(&self) -> LogicalPortFailures {
        self.failure_types
    }
    ...
    pub fn chain<R: Into<ModuleResult>>(&self, next: R) -> Self {
        ...
        Self::new(success, failure, error, failure_types).unwrap()
    }
...
/// right now.
#[derive(Copy, Clone, PartialEq)]
pub struct LogicalPortFailures(pub [FpgaI2CFailure; NUM_PORTS as usize]);

impl Default for LogicalPortFailures {
    fn default() -> Self {
        Self([FpgaI2CFailure::NoError; NUM_PORTS as usize])
    }
}

impl core::ops::Index<LogicalPort> for LogicalPortFailures {
